...
public class StorageGUI {
    ...
    public static final int AUTOSELL_SLOT = 47;
    ...
    private void fillMaterials(Inventory inv, Player player, int page) {
        ...
        inv.setItem(AUTOSELL_SLOT, buildAutosellButton(player));
    ...
    private ItemStack buildAutosellButton(Player player) {
        boolean hasPerm = player.hasPermission("primefarm.autosell") || player.hasPermission("primefarm.admin");

        if (!hasPerm) {
            ItemStack item = new ItemStack(Material.BARRIER);
            ItemMeta meta = item.getItemMeta();
            meta.setDisplayName(plugin.getLanguageManager().get(player, "gui-autosell-locked-name"));
            meta.setLore(plugin.getLanguageManager().getList(player, "gui-autosell-locked-lore"));
            item.setItemMeta(meta);
            return item;
        }

        boolean enabled = plugin.getAutoSellManager().isEnabled(player.getUniqueId());
        // Not: REDSTONE_TORCH_OFF bu surumde envanter itemi olarak gecersiz (gorunmez oluyor),
        // bu yuzden tek, her zaman gecerli bir materyal (LEVER) kullanip durumu sadece
        // renk/isimle gosteriyoruz.
        ItemStack item = new ItemStack(Material.LEVER);
        ItemMeta meta = item.getItemMeta();
        meta.setDisplayName(plugin.getLanguageManager().get(player, enabled ? "gui-autosell-on-name" : "gui-autosell-off-name"));
        meta.setLore(plugin.getLanguageManager().getList(player, "gui-autosell-lore"));
        item.setItemMeta(meta);
        return item;
    }
